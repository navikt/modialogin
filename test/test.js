...
test('proxying to protected endpoint when logged in', async () => {
    ...
    assertThat(protectedEndpoint.body.headers['cookie'], startsWith('frontend_sessionid'), '/frontend sent frontend_sessionid cookie');
});
...
test('proxying to protected endpoint when logged in, and rewriting cookie name', async () => {
    ...
    assertThat(protectedEndpoint.body.headers['cookie'], notContains('frontend_sessionid'), '/frontend did not send frontend_sessionid cookie');
});
...
async function azureadloginflow(port) {
    ...
    const sessionCookie = loginCookies.find(cookie => cookie.startsWith('frontend_sessionid'));
...
async function createSessionId(tokens) {
    ...
    await fetch(`http://localhost:8080/redis/session/${sessionId}`, {}, data);
    return sessionId;
}